...
这里选 GitHub Actions 做例子是因为便于理解，但沙盒演练真正的价值在于**内部工具和非标准系统**。GitHub Actions 有完善的官方文档，LLM 训练数据里也见过大量相关内容，agent 即使不演练也能给出不错的回答。但团队内部的部署系统、自研的数据管道、私有的存储服务——这些东西没有公开文档，LLM 的先验知识几乎为零。这时候沙盒演练就是 agent 相比纯 LLM 的核心优势：它能实际运行命令、观察输出、根据反馈调整，通过与环境的交互来弥补训练数据的空白。对于这类工具，一次沙盒演练产出的 skill 可能比读十遍内部文档都有用。
